...
human_df["paragraph_text"] = human_df["full_text"].str.split("\n")

# 4) 빈 문자열 제거 + strip(앞뒤 공백 제거)
human_df["paragraph_text"] = human_df["paragraph_text"].apply(
    lambda lst: [p.strip() for p in lst if p.strip()]
)

# 5) explode → 한 문단(문장)씩 행으로 펼치기
exploded = human_df.explode("paragraph_text").reset_index(drop=True)
...
out_cols = ["title", "paragraph_index", "paragraph_text", "generated"]
